...
RSpec.describe "AddCorrelationIdToCacheEvents migration" do
  let(:migration_file) do
    File.join(__dir__, "../../../db/migrate/20260630000001_add_correlation_id_to_cache_events.rb")
  end

  let(:migration_class) { AddCorrelationIdToCacheEvents }
  ...
  after do
    connection = ActiveRecord::Base.connection
    connection.drop_table(:solid_observer_cache_events) if connection.table_exists?(:solid_observer_cache_events)
  end
  ...
  describe "up migration" do
    it "adds correlation_id column with limit 64" do
      ...
      columns = ActiveRecord::Base.connection.columns(:solid_observer_cache_events)
    ...
    it "creates partial index on correlation_id" do
      ...
      indexes = ActiveRecord::Base.connection.indexes(:solid_observer_cache_events)
  ...
  describe "down migration" do
    it "removes correlation_id column and index" do
      ...
      columns = ActiveRecord::Base.connection.columns(:solid_observer_cache_events).map(&:name)
